...
class GeneticSelectionPmdarimaCV(BaseEstimator, MetaEstimatorMixin, SelectorMixin):
    ...
    def _fit(self, X, y):
        ...
        final_scores = model_selection.cross_val_score(estimator=self.estimator_, X=X[:, support_], y=y, scoring=scorer, cv=cv)
        final_scores_mean = np.mean(final_scores)
        final_scores_std = np.std(final_scores)

        print(f'Final scores: {final_scores_mean}, SD: {final_scores_std}')
